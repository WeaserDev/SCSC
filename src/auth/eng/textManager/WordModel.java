...
public abstract class WordModel {
	...
	private HashMap<Class<?>, Set<String>> stopWordStemmedSetPerStemmer = new HashMap<Class<?>, Set<String>>();
	...
	final protected static String prepareAcronyms(String sentence) {
		...
		for(int i=0;i<s.length;i++)
			if((i==0||!Character.isAlphabetic(s[i-1])||Character.isUpperCase(s[i-1])) && (i==s.length-1||Character.isUpperCase(s[i+1])||!Character.isAlphabetic(s[i+1])) && Character.isUpperCase(s[i]))
