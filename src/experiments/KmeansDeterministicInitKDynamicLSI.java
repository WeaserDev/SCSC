...
public class KmeansDeterministicInitKDynamicLSI extends Experiment {
	...
	public void test(ProjectInput project, WordModel wordModel) throws IOException {
		...
		MergePackagesToClusters pack = new MergePackagesToClusters(project.getProjectDirectory(),5);
						...
						for (DistanceFunction distance:distances) {
							for (int closestCentroids=1; closestCentroids<=maxClosestCentroids;closestCentroids++) {
								...
								for (int t=0; t<occurences.length;t++) {
									for (int clusterNumber=10;clusterNumber<=project.getInput().length/4;clusterNumber++) {
