...
public class KmeansDeterministicInitKKnownOccurence extends Experiment {
	...
	public void test(ProjectInput project, WordModel wordModel) throws IOException {
		...
		LocalWeightMethod localWeights[] = {new LogWeight(), new TermFrequencyWeight()};
		...
		DistanceFunction[] distances = {new clustering.distance.ModifiedCosineDistance()};
		int maxFileWeight = 5;
		...
		for (WeightMethod weight:weights) {
			for (float referenceWeight=0; referenceWeight<=maxReferenceWeight; referenceWeight+=referenceWeightStep) {
				for (int fileWeight=5;fileWeight<=maxFileWeight;fileWeight+=fileWeightStep) {
					for (int functionWeight=3;functionWeight<=maxFunctionWeight;functionWeight+=functionWeightStep) {
						...
						for (DistanceFunction distance:distances) {
							for (int closestCentroids=1; closestCentroids<=maxClosestCentroids;closestCentroids++) {
								...
								LSA[] lsis = {new LSA(occurence,90.0f), new LSA(occurence,95.0f), new LSA(occurence, factors), new LSA(occurence)};
								float occurences[][][] = {occurence, lsis[0].getDocumentConceptTable(), lsis[0].getLowRankApproximation(), lsis[1].getDocumentConceptTable(),lsis[1].getLowRankApproximation(),lsis[2].getDocumentConceptTable(),lsis[2].getLowRankApproximation(),lsis[3].getDocumentConceptTable(),lsis[3].getLowRankApproximation()};
								String occurenceMethods[] = {"initial table","90% concept","90% approx", "95% concept", "95% aprox", "n*m^0.2 concept", "n*m^0.2 aprox", "kaiser concept","kaiser aprox"};
