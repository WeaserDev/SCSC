...
public class KmeansDeterministicInitKKnownDistanceWeightLSIDC extends Experiment {
	 ...
	 * Features are extracted using WordModelFeatureExtractionReferenceAddedWeight. Weights and functions.
	 * @throws IOException 
	 */
	@Override
	public void test(ProjectInput project, WordModel wordModel) throws IOException {
		...
		GlobalWeightMethod globalWeights[] = {new EntropyWeight(), new GfIdfWeight(), new InverseDocumentFrequencyWeight(), new NormalWeight(), new NoGlobalWeight()};		
		...
		DistanceFunction[] distances = {new CosineDistance(), new clustering.distance.ModifiedCosineDistance(), new clustering.distance.DotDistance(), new clustering.distance.EllenbergDistance(), new clustering.distance.UnbiasedEllenbergDistance()};
		int maxFileWeight = 2;
